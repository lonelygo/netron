    ...
    _buildTable(type) {
        ...
        this._builder.indent();
            ...
            if (type === this._root.get('root_type')) {
                ...
                this._builder.indent();
                    this._builder.add('return ' + name + '.decode(reader, reader.int32(reader.position) + reader.position);');
            ...
            this._builder.indent();
                ...
                this._builder.add('let offset;');
                for (const field of type.fields.values()) {
                    this._builder.add('offset = reader.offset(position, ' + field.offset + ');');
                    if (field.repeated) {
                        if (field.type instanceof flatc.PrimitiveType && field.type.name !== 'string' && field.type.name !== 'int64' && field.type.name !== 'uint64' && field.type.name !== 'bool') {
                            ...
                            this._builder.add('$.' + field.name + ' = offset ? new ' + arrayType + '(reader.buffer.buffer, reader.buffer.byteOffset + reader.vector(position + offset), reader.length(position + offset)) : null;');
                        }
                            ...
                            this._builder.add('$.' + field.name + ' = reader.array(offset, position, ' + fieldType + '.decode);');
                        }
                    }
                    else {
                        if (field.type instanceof flatc.PrimitiveType) {
                            this._builder.add('$.' + field.name + ' = offset ? reader.' + field.type.name + '(position + offset) : ' + field.defaultValue + ';');
                        }
                        else if (field.type instanceof flatc.Enum) {
                            const fieldType = field.type instanceof flatc.Enum ? field.type.base : field.type;
                            this._builder.add('$.' + field.name + ' = offset ? reader.' + fieldType.name + '(position + offset) : ' + field.defaultValue + ';');
                        }
                        else if (field.type instanceof flatc.Union) {
                            this._builder.add('const $type$' + field.name + '= offset ? reader.uint8(position + offset) : 0;');
                            this._builder.add('offset = reader.offset(position, ' + (field.offset + 2) + ');');
                            const unionType = '$root.' + field.type.parent.name + '.' + field.type.name;
                            this._builder.add('$.' + field.name + ' = offset ? ' + unionType + '.decode(reader, position + offset, $type$' + field.name + ') : null;');
                        }
                        else if (field.type instanceof flatc.Struct) {
                            this._builder.add('// TODO Struct');
                            this._builder.add('$.' + field.name + '= undefined;');
